...
export class NewsManager {
  ...
  addArticle(article: Omit<NewsArticle, 'id' | 'fetchedAt' | 'read' | 'archived'>): NewsArticle {
    ...
    if (existing) {
      return this.getArticle((existing as { id: string }).id)!;
    }
  ...
  getArticle(id: string): NewsArticle | undefined {
    ...
    const row = stmt.get(id) as NewsArticleRow | undefined;
    return row ? this.mapRowToArticle(row) : undefined;
  }

  listArticles(filter?: { category?: string; source?: string; read?: boolean; archived?: boolean; limit?: number; offset?: number }): NewsArticle[] {
    ...
    const rows = stmt.all(...params) as NewsArticleRow[];
  ...
  markAsRead(id: string): NewsArticle | undefined {
    ...
    const historyStmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?)
    `);
    historyStmt.run(uuidv4(), id, new Date().toISOString());
  ...
  cleanupOldArticles(daysToKeep = 7): number {
  ...
  listSources(enabledOnly = false): NewsSource[] {
  ...
  getReadingHistory(limit = 50): ReadingHistory[] {
    const stmt = this.db.prepare('SELECT * FROM reading_history ORDER BY read_at DESC LIMIT ?');
    const rows = stmt.all(limit) as ReadingHistoryRow[];
    return rows.map(row => ({
      id: row.id,
  ...
  private mapRowToArticle(row: NewsArticleRow): NewsArticle {
    return {
      id: row.id,
      title: row.title,
      content: row.content,
  ...
  private mapRowToSource(row: NewsSourceRow): NewsSource {
    return {
      id: row.id,
      name: row.name,
      url: row.url,
  ...
  private mapRowToPreference(row: UserPreferenceRow): UserPreference {
    return {
      key: row.key,
      value: row.value,
