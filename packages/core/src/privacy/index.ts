...
export class DataPrivacyManager {
  ...
  async restore(backupPath: string, targetDirectory?: string): Promise<void> {
  ...
  verifyNoCloudSync(): { passed: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  private copyDirectoryRecursive(src: string, dest: string, excludePatterns?: string[]): void {
