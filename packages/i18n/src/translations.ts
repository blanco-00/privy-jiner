export const en = {
  common: {
    appName: 'Privy-Jiner',
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    search: 'Search',
    confirm: 'Confirm',
    back: 'Back',
    next: 'Next',
    previous: 'Previous',
    close: 'Close',
    success: 'Success',
    error: 'Error',
    warning: 'Warning',
    info: 'Info',
    yes: 'Yes',
    no: 'No',
  },
  login: {
    pureLoginSuccess: 'Login Successful',
    pureLoginFail: 'Login Failed',
    pureUsernameReg: 'Username is required',
    pureUsername: 'Username',
    purePassword: 'Password',
    pureVerifyCode: 'Verification Code',
    pureRemember: 'Remember me',
    pureRememberInfo: 'For security, please log out on shared devices',
    pureForget: 'Forgot password?',
    pureLogin: 'Login',
    purePhoneLogin: 'Phone Login',
    pureQRCodeLogin: 'QR Code Login',
    pureRegister: 'Register',
    pureThirdLogin: 'Third Party Login',
    pureWeChatLogin: 'WeChat Login',
    pureAlipayLogin: 'Alipay Login',
    pureQQLogin: 'QQ Login',
    pureWeiBoLogin: 'Weibo Login',
  },
  nav: {
    dashboard: 'Dashboard',
    finance: 'Finance',
    health: 'Health',
    fashion: 'Fashion',
    knowledge: 'Knowledge',
    news: 'News',
    settings: 'Settings',
    plugins: 'Plugins',
  },
  dashboard: {
    welcome: 'Welcome to Privy-Jiner',
    overview: 'Overview',
    quickActions: 'Quick Actions',
    recentActivity: 'Recent Activity',
    todayStats: "Today's Stats",
  },
  finance: {
    income: 'Income',
    expense: 'Expense',
    balance: 'Balance',
    budget: 'Budget',
    investment: 'Investment',
    bill: 'Bill',
    category: 'Category',
    amount: 'Amount',
    date: 'Date',
    description: 'Description',
    addRecord: 'Add Record',
    addBudget: 'Add Budget',
    addInvestment: 'Add Investment',
    addBill: 'Add Bill',
    monthlyReport: 'Monthly Report',
    categoryBreakdown: 'Category Breakdown',
  },
  health: {
    water: 'Water',
    exercise: 'Exercise',
    goal: 'Goal',
    reminder: 'Reminder',
    dailyGoal: 'Daily Goal',
    weeklyGoal: 'Weekly Goal',
    logWater: 'Log Water',
    logExercise: 'Log Exercise',
    addReminder: 'Add Reminder',
    todayProgress: "Today's Progress",
    weeklyStats: 'Weekly Stats',
  },
  fashion: {
    wardrobe: 'Wardrobe',
    outfit: 'Outfit',
    capsule: 'Capsule Wardrobe',
    recommendation: 'Recommendation',
    addItem: 'Add Item',
    logOutfit: 'Log Outfit',
    createCapsule: 'Create Capsule',
    getRecommendation: 'Get Recommendation',
  },
  knowledge: {
    knowledgeBase: 'Knowledge Base',
    dailyPush: 'Daily Push',
    category: 'Category',
    addEntry: 'Add Entry',
    schedulePush: 'Schedule Push',
    pushHistory: 'Push History',
  },
  news: {
    articles: 'Articles',
    sources: 'Sources',
    preferences: 'Preferences',
    addSource: 'Add Source',
    refresh: 'Refresh',
    readMore: 'Read More',
    trending: 'Trending',
  },
  settings: {
    general: 'General',
    language: 'Language',
    theme: 'Theme',
    database: 'Database',
    backup: 'Backup',
    security: 'Security',
    about: 'About',
    selectLanguage: 'Select Language',
    english: 'English',
    chinese: '中文',
  },
  plugins: {
    installed: 'Installed',
    available: 'Available',
    install: 'Install',
    uninstall: 'Uninstall',
    enable: 'Enable',
    disable: 'Disable',
    noPlugins: 'No plugins installed',
  },
  errors: {
    notFound: 'Not Found',
    unauthorized: 'Unauthorized',
    serverError: 'Server Error',
    networkError: 'Network Error',
    invalidInput: 'Invalid Input',
  },
};

export const zh = {
  common: {
    appName: '瑾儿私人管家',
    loading: '加载中...',
    save: '保存',
    cancel: '取消',
    delete: '删除',
    edit: '编辑',
    add: '添加',
    search: '搜索',
    confirm: '确认',
    back: '返回',
    next: '下一步',
    previous: '上一步',
    close: '关闭',
    success: '成功',
    error: '错误',
    warning: '警告',
    info: '信息',
    yes: '是',
    no: '否',
  },
  login: {
    pureLoginSuccess: '登录成功',
    pureLoginFail: '登录失败',
    pureUsernameReg: '用户名不能为空',
    pureUsername: '用户名',
    purePassword: '密码',
    pureVerifyCode: '验证码',
    pureRemember: '记住我',
    pureRememberInfo: '为了您的账号安全，请勿在公共设备上保留登录状态',
    pureForget: '忘记密码？',
    pureLogin: '登录',
    purePhoneLogin: '手机登录',
    pureQRCodeLogin: '二维码登录',
    pureRegister: '注册',
    pureThirdLogin: '第三方登录',
    pureWeChatLogin: '微信登录',
    pureAlipayLogin: '支付宝登录',
    pureQQLogin: 'QQ登录',
    pureWeiBoLogin: '微博登录',
  },
  nav: {
    dashboard: '控制台',
    finance: '财务',
    health: '健康',
    fashion: '时尚',
    knowledge: '知识',
    news: '新闻',
    settings: '设置',
    plugins: '插件',
  },
  dashboard: {
    welcome: '欢迎使用瑾儿私人管家',
    overview: '概览',
    quickActions: '快捷操作',
    recentActivity: '最近活动',
    todayStats: '今日统计',
  },
  finance: {
    income: '收入',
    expense: '支出',
    balance: '余额',
    budget: '预算',
    investment: '投资',
    bill: '账单',
    category: '分类',
    amount: '金额',
    date: '日期',
    description: '描述',
    addRecord: '添加记录',
    addBudget: '添加预算',
    addInvestment: '添加投资',
    addBill: '添加账单',
    monthlyReport: '月度报告',
    categoryBreakdown: '分类明细',
  },
  health: {
    water: '喝水',
    exercise: '运动',
    goal: '目标',
    reminder: '提醒',
    dailyGoal: '每日目标',
    weeklyGoal: '每周目标',
    logWater: '记录喝水',
    logExercise: '记录运动',
    addReminder: '添加提醒',
    todayProgress: '今日进度',
    weeklyStats: '每周统计',
  },
  fashion: {
    wardrobe: '衣橱',
    outfit: '穿搭',
    capsule: '胶囊衣柜',
    recommendation: '推荐',
    addItem: '添加衣物',
    logOutfit: '记录穿搭',
    createCapsule: '创建胶囊',
    getRecommendation: '获取推荐',
  },
  knowledge: {
    knowledgeBase: '知识库',
    dailyPush: '每日推送',
    category: '分类',
    addEntry: '添加条目',
    schedulePush: '设置推送',
    pushHistory: '推送历史',
  },
  news: {
    articles: '文章',
    sources: '来源',
    preferences: '偏好',
    addSource: '添加来源',
    refresh: '刷新',
    readMore: '阅读更多',
    trending: '热门',
  },
  settings: {
    general: '通用',
    language: '语言',
    theme: '主题',
    database: '数据库',
    backup: '备份',
    security: '安全',
    about: '关于',
    selectLanguage: '选择语言',
    english: 'English',
    chinese: '中文',
  },
  plugins: {
    installed: '已安装',
    available: '可用',
    install: '安装',
    uninstall: '卸载',
    enable: '启用',
    disable: '禁用',
    noPlugins: '暂无插件',
  },
  errors: {
    notFound: '未找到',
    unauthorized: '未授权',
    serverError: '服务器错误',
    networkError: '网络错误',
    invalidInput: '输入无效',
  },
};

export type Locale = 'en' | 'zh';
export type TranslationKeys = typeof en;
